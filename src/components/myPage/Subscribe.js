...
import SubscribeTermPopup from '../popup/SubscribeTermPopup';
...
const Subscribe = (props) => {
    ...
    const subscribe_term_popUp_visible = ()=>{
        props.propFunction(1)
        props.propFunction3()
        DUMMY_SUBSCRIBE.map((item) => {
            if(item.productName == props.subscribeInfo){
                item.productCycle = props.subscribeTerm
                console.log(item.productCycle)
            }
        })
    ...
    <div className="container" style={{color:'black', borderColor:'black'}}>
        <div className="top-area">
            <div className="row align-items-center">
                ...
                {DUMMY_SUBSCRIBE.map((item) => (
                    <div className="single-item" style={{color:'black', paddingBottom:'10px', display:deleteSub}}>
                        <div className="row row2">
                            ...
                            <div className="col-lg-2 col-12" style={{paddingTop: '35px'}}>
                                <div>{props.subscribeTerm != undefined && item.productName == props.subscribeInfo ? props.subscribeTerm : item.productCycle}일</div>
                            </div>
                        ...
                        <div style={{float: 'right', marginTop: '-30px'}}>
                            <button className="use-coupon use-coupon2" onClick={()=>{subscribe_term_popUp_visible(); props.propFunction2(item.productName);}}>자동 구매 주기 변경</button>
                            <button className="use-coupon use-coupon2" onClick={()=> setDeleteSub('none')}>자동 구매 취소</button>
                            <button className="use-coupon use-coupon2">관련상품</button>
                        </div>
                    </div>
                ))}
            </div>
